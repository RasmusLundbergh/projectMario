...
public class Menu {
    ...
    public void visHovedMenu() {
        ...
        while (kører) {
            ...
            System.out.println("5. Se statistik og omsætning");
    ...
    private void countPizzaOrders() {
        ArrayList<Pizza> countedPizzas = new ArrayList<>();
        ArrayList<Integer> pizzaCounts = new ArrayList<>();

        for (Bestilling ordre : afsluttedeOrdre) {
            for (Pizza pizza : ordre.getPizzaer()) {
                int index = countedPizzas.indexOf(pizza);

                if (index == -1) {
                    countedPizzas.add(pizza);
                    pizzaCounts.add(1);
                } else {
                    pizzaCounts.set(index, pizzaCounts.get(index) + 1);
                }
            }
            if (countedPizzas.isEmpty()) {
                System.out.println("Ingen pizzaer er blevet bestilt endnu.");
            } else {
                for (int i = 0; i < countedPizzas.size(); i++) {
                    Pizza pizza = countedPizzas.get(i);
                    int count = pizzaCounts.get(i);
                    System.out.println("Pizza: " + pizza.getNavn() + " er blevet bestilt " + count + " gange");
                }
            }
        }
        }

    private void Statistik_og_omsætning() {
        //System.out.println(afsluttedeOrdre.getFirst().getPris() + "Kr");
        double omsætning = 0.0;
        for (int i = 0; i < afsluttedeOrdre.size(); i++) {
            omsætning += afsluttedeOrdre.get(i).getPris();
        }
        System.out.println("Omsætning = " + omsætning + " Kr.");
        countPizzaOrders();
            }
}